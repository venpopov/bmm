...
#'
#' @param object An object of class \code{bmmfit}.
#' @param prob_ci A value between 0 and 1 indicating the desired probability
#'   to be covered by the posterior credibility interval. The default is 0.95.
...
#' @export
summary.bmmfit <- function(object, prob_ci = 0.95, priors = FALSE,
                           robust = FALSE, mc_se = FALSE, ...) {
  priors <- brms:::as_one_logical(priors)
  robust <- brms:::as_one_logical(robust)
  mc_se <- brms:::as_one_logical(mc_se)
  probs <- validate_ci_bounds(prob_ci)
  object <- brms::restructure(object)
  bterms <- brms::brmsterms(object$formula)
  out <- list(
    formula = object$formula,
    data_name = brms:::get_data_name(object$data),
    group = unique(object$ranef$group),
    nobs = nobs(object),
    ngrps = brms::ngrps(object),
    autocor = object$autocor,
    prior = brms::empty_prior(),
    algorithm = brms:::algorithm(object)
  )
  ...
  variables <- brms::variables(object)
  incl_classes <- c(
    "b", "bs", "bcs", "bsp", "bmo", "bme", "bmi", "bm",
    brms:::valid_dpars(object),
    "delta", "lncor", "rescor", "ar", "ma", "sderr",
  ...
  draws <- brms::as_draws_array(object, variable = variables)
  full_summary <- .summary(draws, variables, probs, robust)
  if (brms:::algorithm(object) == "sampling") {
    ...
    div_trans <- sum(brms::nuts_params(object, pars = "divergent__")$Value)
    adapt_delta <- brms::control_params(object)$adapt_delta
    if (div_trans > 0) {
      warning2(
  ...
  cor_pars <- variables[grepl(brms:::regex_autocor_pars(), variables)]
...
# regex to extract only parameters included in the model
get_model_pars <- function(model) {
  model_pars <- names(model$info$parameters)
  model_pars <- model_pars[!model_pars %in% names(model$info$fixed_parameters)]
  paste0("(", model_pars, "_)", collapse = "|")
}
