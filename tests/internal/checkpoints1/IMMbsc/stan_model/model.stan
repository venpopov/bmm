...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N) {
      // compute non-linear predictor values
      theta2[n] = (C_theta2_1[n] * (exp( - nlp_expS[n] * C_theta2_2[n]) * nlp_c[n]) + (1 - C_theta2_1[n]) * ( - 100));
    }
    for (n in 1:N) {
      // compute non-linear predictor values
      theta3[n] = (C_theta3_1[n] * (exp( - nlp_expS[n] * C_theta3_2[n]) * nlp_c[n]) + (1 - C_theta3_1[n]) * ( - 100));
    }
    for (n in 1:N) {
      // compute non-linear predictor values
      theta4[n] = (C_theta4_1[n] * (exp( - nlp_expS[n] * C_theta4_2[n]) * nlp_c[n]) + (1 - C_theta4_1[n]) * ( - 100));
    }
    for (n in 1:N) {
