...
data {
  ...
  int<lower=1> K_s;  // number of population-level effects
  matrix[N, K_s] X_s;  // population-level design matrix
...
transformed parameters {
  ...
  vector[K_s] b_s;  // regression coefficients
  ...
  b_a[1] = 0;
  lprior += normal_lpdf(b_a[2] | 0,1);
  lprior += normal_lpdf(b_a[3] | 0,1);
  lprior += normal_lpdf(b_a[4] | 0,1);
  lprior += normal_lpdf(b_c | 0,1);
  b_s[1] = 0;
  lprior += normal_lpdf(b_s[2] | 0,1);
  lprior += normal_lpdf(b_s[3] | 0,1);
  lprior += normal_lpdf(b_s[4] | 0,1);
}
model {
  ...
  if (!prior_only) {
    ...
    // initialize linear predictor term
    vector[N] nlp_c = rep_vector(0.0, N);
    // initialize linear predictor term
    vector[N] nlp_s = rep_vector(0.0, N);
    ...
    for (n in 1:N) {
      // compute non-linear predictor values
      theta1[n] = (nlp_c[n] + nlp_a[n]);
    }
    for (n in 1:N) {
      // compute non-linear predictor values
      theta2[n] = (C_theta2_1[n] * (exp( - nlp_expS[n] * C_theta2_2[n]) * nlp_c[n] + nlp_a[n]) + (1 - C_theta2_1[n]) * ( - 100));
    }
    for (n in 1:N) {
      // compute non-linear predictor values
      theta3[n] = (C_theta3_1[n] * (exp( - nlp_expS[n] * C_theta3_2[n]) * nlp_c[n] + nlp_a[n]) + (1 - C_theta3_1[n]) * ( - 100));
    }
    for (n in 1:N) {
      // compute non-linear predictor values
      theta4[n] = (C_theta4_1[n] * (exp( - nlp_expS[n] * C_theta4_2[n]) * nlp_c[n] + nlp_a[n]) + (1 - C_theta4_1[n]) * ( - 100));
    }
    for (n in 1:N) {
