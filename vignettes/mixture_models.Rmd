...
knitr::include_graphics("mixture_models_illustration.jpg")
```
...
- `setsize` is the number of items presented in the memory array
...
dat_preprocessed <- dat %>%
  mutate(error = wrap(response - target),
         ...
         set_size = paste0('Set size ', set_size))
...
```{r}
model <- mixture2p(respErr = "error")
```

Finally, we fit the model with the `fit_model` function. The fit model function uses
...
fit <- readRDS(system.file("mixture2p_vignette_fit.rds", package = "bmm"))
```
...
kappa <- exp(kappa)
sd <- k2sd(kappa[,1]) 
pmem <- exp(thetat)/(exp(thetat)+1)
pg <- exp(0)/(exp(thetat)+1)
```

Our estimates for kappa over setsize are:
...
```{r}
rownames(pmem) <- paste0("Set size ", c(1,2,4,6))
round(pmem,3)
```

It is even better to visualize the entire posterior distribution of the parameters. 

```{r, message=FALSE, warning=FALSE, fig.show='hold', fig.width=4, fig.height=4}
library(tidybayes)
library(tidyr)
...
as.data.frame(draws_kappa) %>% 
  ...
  mutate(par = gsub("b_kappa_set_size", "Set size ", par)) %>%
  ggplot(aes(par,value)) +
  tidybayes::stat_halfeyeh(normalize="groups", orientation="vertical") +
...
model <- mixture3p(non_targets = paste0('non_target_',1:5), setsize='set_size')
```

Then we run the model just like before:

``` r
fit3p <- fit_model(formula = ff,
data = dat_preprocessed,
model = model,
parallel=T,
iter=2000,
