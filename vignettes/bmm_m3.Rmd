...
In addition, the `m3` allows to include additional hypothetical processes that might increase or diminishing these activation strengths due to experimental manipulations, such as manipulating encoding time or distraction.
...
In this case, $b$ is the baseline activation, that can be understood as the activation of all digits, because participants now that they have to remember digits in general. The parameter $a$ is the memory strength for items, or general activation, for all items that need to be remembered in the current trial. And parameter $c$ is the memory strength for relations, or context activation, that arises from the context cue of retrieving the cued serial position.
...
After having normalized the activation of each category into probabilities the model than links the response frequencies of each response categories $Y$ to the probabilities assuming a Multinomial distributions with the total number of $trials$ and the derived probabilities $p$:
...
The `version = "custom"` of the `m3` allows you to specify custom activation functions for memory tasks beyond simple and complex span tasks.[^2] For example, you could use a custom `m3` to dissociate different activation sources and cognitive processes contributing to memory performance in memory updating tasks. In addition, to recalling the correct item, items from other positions, or items not presented at all during the current trial, in such tasks participants can also recall _outdated items_ that have been updated during the trial. The frequency of recalling such items, can inform us of the processes contributing towards replacing the initially encoded item with the new item. One such model is discussed in @oberauerSimpleMeasurementModels2019.
...
Generally, one of the `m3` parameters has to be fixed to set the scaling of the model. In `bmm` the default is to fix the background noise parameter `b`. Thus the values of the other parameters should be interpreted relative to the background noise. The value `b` will be fixed to depends on the choice rule you choose for fitting the model. For `luce`, `b` will be fixed to `0.1`, for `softmax`, `b` will be fixed to `0`.

We chose two fix the background noise `b` for scaling for two reasons:

1. Fixing the background noise is similar to fixing noise parameters in other cognitive measurement models, such as the diffusion constant `s`, or the standard deviation of the noise added to the signals in SDT.
...
For the `ss` and `cs` version of the `m3` these steps are practically identical to fitting any other `bmmodel`. The `custom` version of the `m3`
requires some additional `bmmformula` to specify the custom activation formulas for each response category. This will be explained below.
...
How you label the parameters in the activation formula is  up to you. We recommend using short labels to avoid parsing errors with special symbols. There is _one_ exception with respect to the naming: The parameter `b` is reserved for the baseline activation and also required to be part of the activation function of each response category.
...
- the number of response options in each category
- the choice rule, if not explicitly called `bmm` will use the `softmax` to fit the `m3`
- the `m3` version, if not explicitly selected `bmm` will use the `custom` version
...
For the `custom` version of the, you additionally need to provide the `links` that should be used for each of the model parameters. We also recommend that you provide  `default_priors` for each model parameter Otherwise, they will be set based on the provided `links`. 

The `links` ensure that model parameters stay in the correct value range. In particular for the `luce` choice rule it is essential, that all activation are positive. Thus, we recommend using `log` link functions for all parameters that represent activation sources, such as general or context activation.
...
Setting up a `m3` object including these infos looks like this:
...
my_priors <- list(
  c = list(main = "normal(1.5, 0.5)", effect = "normal(0, 0.5)"),
...
Running this model takes about 20 to 40 seconds (depending on the speed of your computer). Here we load an already saved fit object, but on your machine you will have to wait until the model finishes sampling. Both `brms` and `cmdstanr` typically print out some information on the sampling progress.

```{r}
fit <- bmm:::bmmfit_m3_vignette
```
...
These estimates differ from the estimates reported by @oberauerSimpleMeasurementModels2019, becasue we used a `log` link function, whereas in the original publication an `identity` link was used. This was done with adding truncation arguments to the priors, to ensure that all activations are positive. In principle, this is possible in `bmm` to. However, such an estimation is less numerically stable and efficient. Therefore, we recommend using `log` links for activation parameters.
...
```{r}
post_diff <- c(
  c_newVoldR = "exp(c_condnewdistractors) = exp(c_condoldreordered)",
  c_newVoldS = "exp(c_condnewdistractors) = exp(c_condoldsame)",
  c_oldRVolds = "exp(c_condoldreordered) = exp(c_condoldsame)",
  a_newVoldR = "exp(a_condnewdistractors) = exp(a_condoldreordered)",
  a_newVoldS = "exp(a_condnewdistractors) = exp(a_condoldsame)",
  a_oldRVolds = "exp(a_condoldreordered) = exp(a_condoldsame)"
)

hypothesis(fit, post_diff)
```

These hypothesis will compute the difference between the specified parameters. To ease interpretability, we have transformed them back to the native scale using the `exp` function in defining the comparisons. You can see that the `c` parameter in the condition `oldsame` differs from both the `new` and `oldreorderd` although the 95% credibility interval (CI) includes zero.  In contrast the `a` parameter in the condition `oldreordered` differs from both the `new` and the `oldsame` condition. For these differences the 95% CI does not include zero as indicated by the `*` at the end of the line.

Had we included a statement to also sample the priors using the `sample_prior = TRUE` option from `brms` when running the `bmm`, the `Evid.Ratio` and `Post.Prob` column would have given us the Bayes Factor in favor of the hypothesis and the posterior probability for the hypothesis being true relative to the specified priors.
